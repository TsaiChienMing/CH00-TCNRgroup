...
public class DetailContentProvider extends ContentProvider {
    private static final String AUTHORITY = "as.traveler.ast_detail1001";
    private static final String DB_FILE = "detail.db", DB_TABLE = "detail";
    ...
    private SQLiteDatabase mdetailDb;

    private static final String crTBsql = "CREATE TABLE " + DB_TABLE + " ( "
            + "id INTEGER PRIMARY KEY," + "sch_id TEXT NOT NULL," + "day TEXT,"+ "att_id TEXT);";
    ...
    public boolean onCreate() {
        ...
        DbOpenHelper detailDbOpenHelper = new DbOpenHelper(getContext(), DB_FILE, null, 1);

        mdetailDb = detailDbOpenHelper.getWritableDatabase();
        // 檢查資料表是否已經存在，如果不存在，就建立一個。
        Cursor cursor = mdetailDb
                .rawQuery("select DISTINCT tbl_name from sqlite_master where tbl_name = '" + DB_TABLE + "'", null);
        if (cursor != null) {
            if (cursor.getCount() == 0) // 沒有資料表，要建立一個資料表。
                mdetailDb.execSQL(crTBsql);
            cursor.close();
        }
        return true;
    }
...
//  Cursor c = mdetailDb.query(distinct, table, columns, selection, selectionArgs, groupBy, having, orderBy, limit)
        Cursor c = mdetailDb.query(true, DB_TABLE, projection, selection, selectionArgs, null, null, sortOrder, null); //"ASC DESC"
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        long rowId = mdetailDb.insert(DB_TABLE, null, values);
        if (rowId > 0) {
            // 在已有的 Uri的後面追加ID數據
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        int rowsAffected = mdetailDb.delete(DB_TABLE, selection, null);
        return rowsAffected;
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        int rowsAffected = mdetailDb.update(DB_TABLE, values, selection, null);
        return rowsAffected;
    }
    // ------------------------
}
